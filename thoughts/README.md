...
The shared header progressively enhances its navigation: without `reading-nav.js`, links wrap in an in-flow header so enlarged text never clips. The script adds `reading-nav-enhanced` only after keyboard scrolling is installed. Keep the shared CSS and script cache versions synchronized across all reading pages and the Gratitude renderer when changing this behavior.
